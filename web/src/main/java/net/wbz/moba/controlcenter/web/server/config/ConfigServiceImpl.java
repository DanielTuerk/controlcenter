...
public class ConfigServiceImpl extends RemoteServiceServlet implements ConfigService {
    ...
    public String loadValue(String configKey) throws ConfigNotAvailableException {
        TypedQuery<ConfigValue> typedQuery = entityManager.get().createQuery(
                "SELECT x FROM ConfigValue x where key=" + configKey, ConfigValue.class);
        ConfigValue resultList = typedQuery.getSingleResult();
        if (resultList != null) {
